...
// In-Memory Database Store for Web Server & Serverless Function
let activityLogs = [];
let packages = [];
let products = [];
let customers = [];
...
function processRequest(data, res) {
    ...
    if (action === 'kt_change_superadmin_password') {
        ...
        const pkgId = parseInt(data.package_id);
        ...
        if (id > 0) {
            ...
            if (existing) {
                ...
                existing.cnic_id = data.cnic_id;
                existing.area_sector = data.area_sector;
                existing.package_id = pkgId;
                existing.package_name = pkg ? pkg.package_name : existing.package_name;
                existing.assigned_ip_ipoe = data.assigned_ip_ipoe;
                ...
                res.end(JSON.stringify({ success: true, data: { message: 'Subscriber updated successfully!' } }));
            } else {
            ...
            res.end(JSON.stringify({ success: true, data: { message: `Subscriber ${newCust.full_name} registered successfully with ID ${newCust.customer_code}!` } }));
        }
        ...
        if (idx !== -1) {
            ...
            res.end(JSON.stringify({ success: true, data: { message: `Subscriber ${deletedName} deleted successfully.` } }));
        } else {
            res.end(JSON.stringify({ success: false, data: { message: 'Subscriber not found.' } }));
        }
        ...
        const id = parseInt(data.package_id) || 0;
        ...
        if (id > 0) {
            ...
            if (pkg) {
                ...
                pkg.margin = margin;
                activityLogs.unshift({ id: activityLogs.length + 1, user_id: activeUserId, user_name: activeUser, role_level: activeRole, action_type: 'package_update', description: `Updated broadband package: ${pkg.package_name}`, created_at: new Date().toLocaleString() });
                res.end(JSON.stringify({ success: true, data: { message: 'Package updated successfully!' } }));
            }
        } else {
            const newPkg = {
            ...
            activityLogs.unshift({ id: activityLogs.length + 1, user_id: activeUserId, user_name: activeUser, role_level: activeRole, action_type: 'package_create', description: `Created new broadband package: ${newPkg.package_name}`, created_at: new Date().toLocaleString() });
            res.end(JSON.stringify({ success: true, data: { message: 'New broadband package added successfully!' } }));
        }
        ...
        if (inv) {
            ...
            res.end(JSON.stringify({ success: true, data: { message: `Payment of PKR ${paid} recorded successfully for ${inv.invoice_number}!` } }));
        } else {
